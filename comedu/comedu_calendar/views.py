...
def calendar_new(request):
    if request.user.is_manager == False :
        return render(request, 'comedu_calendar/calendar_new_reject.html')
    else:
...
def calendar_edit(request, pk=None, template_name='comedu_calendar/calendar_edit.html'):##pk값이 없으면 None
    ...
    if request.user == calendar.author or request.user.is_manager == True:
...
def calendar_delete_popup(request, pk):
    if request.user.is_manager == True or calendar.author ==  request.user :
